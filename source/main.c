...
/*   Updated: 2025/07/11 13:41:04 by zmourtab         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	color_to_int(const t_color *pixel_color, int samples_per_pixel)
{
	...
	return (((int)(256 * interval_clamp(&intensity,
					linear_to_gamma(scaled_pixel.e[0])))) << 16) | (((int)(256
				* interval_clamp(&intensity,
					linear_to_gamma(scaled_pixel.e[1])))) << 8) | ((int)(256
...
t_ray	get_ray(t_camera *cam, int i, int j)
{
	...
	return (ray_create(cam->center, vec3_subtract(&pixel_sample,
				&cam->center)));
}
...
int	main(void)
{
	...
	cam.vfov = 60;
	cam.lookfrom = vec3_create(4, 1, 1);
	cam.lookat = vec3_create(0, 1, 0);
